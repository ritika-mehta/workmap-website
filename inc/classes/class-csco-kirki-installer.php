			...
			class CSCO_Kirki_Installer extends WP_Customize_Section {
				...
				protected function install_button() {
					...
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'Please install the Kirki plugin to take full advantage of this theme\s customizer capabilities', 'squaretype' ); ?>
					</p>
					<a class="install-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( $this->get_plugin_install_url() ); ?>" aria-label="<?php esc_attr_e( 'Install Kirki Toolkit now', 'squaretype' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Install Now', 'squaretype' ); ?>
					</a>
					<?php
				}
				...
				protected function activate_button() {
					?>
					<p style="text-align:left;margin-top:0;">
						<?php esc_html_e( 'You have installed Kirki. Activate it to take advantage of this theme\'s features in the customizer.', 'squaretype' ); ?>
					</p>
					<a class="activate-now button-primary button" data-slug="kirki" href="<?php echo esc_url_raw( self_admin_url( 'plugins.php' ) ); ?>" aria-label="<?php esc_attr_e( 'Activate Kirki Toolkit now', 'squaretype' ); ?>" data-name="Kirki Toolkit">
						<?php esc_html_e( 'Activate Now', 'squaretype' ); ?>
					</a>
					<?php
				}
				/**
				...
				protected function dismiss_button() {
					...
					$show_confirm = true;
					?>
					<a class="kirki-installer-dismiss button-secondary button" data-slug="kirki" href="#" aria-label="<?php esc_attr_e( 'Don\'t show this again', 'squaretype' ); ?>" data-name="<?php esc_attr_e( 'Dismiss', 'squaretype' ); ?>">
						<?php esc_html_e( 'Don\'t show this again', 'squaretype' ); ?>
					</a>
					...
					jQuery( document ).ready( function() {
						jQuery( '.kirki-installer-dismiss' ).on( 'click', function( event ) {
							...
							<?php if ( $show_confirm ) : ?>
								if ( ! confirm( '<?php esc_html_e( 'Are you sure? Dismissing this message will hide the installation recommendation and you will have to manually install and activate the plugin in the future.', 'squaretype' ); ?>' ) ) {
									return;
								}
							<?php endif; ?>
							jQuery.post( ajaxurl, {
