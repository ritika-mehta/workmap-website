...
function csco_ocdi_import_files() {
	return array(
		array(
			'import_file_name'       => esc_html__( 'Demo Content', 'squaretype' ),
...
function csco_ocdi_page_title() {
	return '<h1>' . esc_html__( 'Demo Content Import', 'squaretype' ) . '</h1>';
}
...
function csco_ocdi_plugin_intro_text( $default_text ) {
	ob_start(); ?>
		<div class="about-description">
			<p><?php esc_html_e( 'Clicking the Import Demo Data button will import demo posts, pages, categories, comments, tags and widgets.', 'squaretype' ); ?></p>
		</div>
	<?php
