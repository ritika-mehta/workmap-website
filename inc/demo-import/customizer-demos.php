...
function register_csco_demos_control() {
	...
	final class CSCO_Demos_Control extends WP_Customize_Control {
		...
		protected function render_content() {
			?>
				<h3><?php esc_html_e( 'Reset', 'squaretype' ); ?></h3>

				<p><?php esc_html_e( 'Click the button to reset the customization settings for this theme.', 'squaretype' ); ?></p>

				<p>
					<input type="submit" name="csco-demos-reset" class="button-secondary button" value="<?php esc_html_e( 'Reset', 'squaretype' ); ?>">
				...
				<?php if ( isset( $demos['demos'] ) && $demos['demos'] ) { ?>
					<h3><?php esc_html_e( 'Select Demo', 'squaretype' ); ?></h3>
									...
									<div class="theme csco-theme-demo" style="width: 100%;" data-demo="<?php echo esc_attr( $slug ); ?>">
										...
										<div class="theme-id-container">
											...
											<div class="theme-actions">
												...
												<a class="button button-primary csco-demo-import" href="#"><?php esc_html_e( 'Activate', 'squaretype' ); ?></a>
											</div>
										</div>
									</div><br>
								<?php
...
class CSCO_Demos_Core {
	...
	public function register( $wp_customize ) {
		...
		$wp_customize->add_section(
			'csco-section', array(
				'title'    => __( 'Theme Demos', 'squaretype' ),
				'priority' => 2,
			)
		);
	...
	public function import_customizer( $slug ) {
		...
		if ( $demos ) {
			...
			if ( isset( $demos['demos'] ) && $demos['demos'] ) {
				// Import Preset.
				if ( isset( $demos['demos'][ $slug ]['preset'] ) && $demos['demos'][ $slug ]['preset'] ) {
					$presets = new CSCO_Presets_Init();

					$presets->set_preset( $demos['demos'][ $slug ]['preset'] );
				}
				// Import Mods.
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', '.csco-demo-import', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: Activating a demo will reset all current customizations.', 'squaretype' ); ?>" );
		...
		<script>
			jQuery(function ($) {
				$('body').on('click', 'input[name="csco-demos-reset"]', function (event) {
					...
					var r = confirm( "<?php esc_html_e( 'Warning: This action will reset all current customizations.', 'squaretype' ); ?>" );
						...
						try {
							...
							if( typeof info.success != 'undefined' && info.success == true ){
								...
								if( typeof info.data != 'undefined' ){
									...
									alert( '<?php esc_html_e( 'Error server!', 'squaretype' ); ?>' );
								}
							}
						} catch (e) {
							alert( response );
